...
AUDIT = logging.AUDIT = logging.INFO + 1
...
class SynapsContextAdapter(logging.LoggerAdapter):
...
def getLogger(name='synaps'):
    if name not in _loggers:
        _loggers[name] = SynapsContextAdapter(logging.getLogger(name))
