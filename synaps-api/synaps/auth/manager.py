...
from synaps.auth import signer
...
auth_opts = [
    ...
    cfg.StrOpt('credential_rc_file',
               ...
               help='Filename of rc in credentials zip %s will be replaced by '
                    'name of the region (synaps by default)'),
    cfg.StrOpt('auth_driver',
               default='synaps.auth.dbdriver.DbDriver',
...
#flags.DECLARE('osapi_compute_listen_port', 'synaps.service')
...
#            drv.delete_user(uid)
    ...
    def __generate_rc(user, pid, use_dmz=True, host=None):
        ...
        if use_dmz:
            cloudwatch_host = FLAGS.cloudwatch_dmz_host
        else:
            cloudwatch_host = FLAGS.cloudwatch_host
        # NOTE(vish): Always use the dmz since it is used from inside the
        #             instance
        if host:
            cloudwatch_host = host
        ...
        rc = rc % {'access': access,
                   'project': pid,
                   'secret': user.secret,
                   'cloudwatch': '%s://%s:%s%s' % (FLAGS.cloudwatch_scheme,
                                                   cloudwatch_host,
                                                   FLAGS.cloudwatch_port,
                                                   FLAGS.cloudwatch_path),
                   'user': user.name,
                   'synaps': FLAGS.ca_file,
