...
class SfJsonEncodeFailure(SynapsException):
...
class VolumeIsBusy(SynapsException):
    message = _("deleting volume %(volume_name)s that has snapshot")


class SnapshotIsBusy(SynapsException):
...
class NetworkNotCreated(SynapsException):
...
class NetworkHostNotSet(SynapsException):
...
class FixedIpAlreadyInUse(SynapsException):
...
class NoMoreFixedIps(SynapsException):
...
class FloatingIpAssociated(SynapsException):
    message = _("Floating ip %(address)s is associated.")


class FloatingIpNotAssociated(SynapsException):
...
class ImageRotationNotAllowed(SynapsException):
    message = _("Rotation is not allowed for snapshots")


class RotationRequiredForBackup(SynapsException):
...
class InvalidSharedStorage(SynapsException):
    message = _("%(path)s is on shared storage: %(reason)s")


class MigrationError(SynapsException):
    message = _("Migration error") + ": %(reason)s"


class MalformedRequestBody(SynapsException):
...
class CannotResizeToSameSize(SynapsException):
    message = _("When resizing, instances must change size!")


class ImageTooLarge(SynapsException):
    message = _("Image is larger than instance type allows")


class ZoneRequestError(SynapsException):
    message = _("1 or more Zones could not complete the request")


class InstanceTypeMemoryTooSmall(SynapsException):
    message = _("Instance type's memory is too small for requested image.")


class InstanceTypeDiskTooSmall(SynapsException):
    message = _("Instance type's disk is too small for requested image.")


class InsufficientFreeMemory(SynapsException):
    message = _("Insufficient free memory on compute node to start %(uuid)s.")


class CouldNotFetchMetrics(SynapsException):
    message = _("Could not fetch bandwidth/cpu/disk metrics for this host.")


class NoValidHost(SynapsException):
    message = _("No valid host was found. %(reason)s")


class WillNotSchedule(SynapsException):
    message = _("Host %(host)s is not up or doesn't exist.")


class QuotaError(SynapsException):
    message = _("Quota exceeded") + ": code=%(code)s"


class AggregateError(SynapsException):
...
class VolumeTypeCreateFailed(SynapsException):
    message = _("Cannot create volume_type with "
                "name %(name)s and specs %(extra_specs)s")


class InstanceTypeCreateFailed(SynapsException):
    message = _("Unable to create instance type")


class SolidFireAPIException(SynapsException):
